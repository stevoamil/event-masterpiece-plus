...
A cinematic, scroll-driven marketing site plus a full admin dashboard for Events By Marina, an event planning & design studio.

## Stack

- **Framework:** Next.js 16 (App Router, TypeScript)
- **Public site:** Three.js + `@react-three/fiber`, GSAP + ScrollTrigger, Lenis smooth scroll, Tailwind CSS
- **Database:** Prisma ORM, SQLite locally (see "Going to production" below)
- **Auth:** Custom JWT session cookies (`jose` + `bcryptjs`) — no third-party auth service
- **AI assistant:** Anthropic API, server-side proxy
...
**Demo admin login:** `admin@eventsbymarina.com` / `EventsByMarina2026!`
**Demo staff login:** `staff@eventsbymarina.com` / `Staff2026!`

## What's fully working out of the box

- The entire public site (hero 3D particle scene, services, portfolio gallery + lightbox, process timeline, testimonials carousel, contact form)
...
|---|---|---|
| AI concierge gives real answers | `ANTHROPIC_API_KEY` | Chat widget shows a friendly fallback message and still logs to the admin dashboard |
...
- **Database:** switch `provider = "sqlite"` to `"postgresql"` in `prisma/schema.prisma`, point `DATABASE_URL` at a hosted Postgres instance (Neon, Supabase, Vercel Postgres), then run `npx prisma migrate deploy`.
